% function dataout = cmpreview(datain)
%
% This function is used to review components generated by different decompositions
...
nsamps=size(datain.cmp,1);
ncomps=size(datain.cmp,2);
ntrials=size(datain.cmp,3);
...
if widthmod==1;
    thefig=figure('pos',[100 100 screensize(3)-200 screensize(4)-250],...
        'name','Component Review','toolbar','figure','menubar','none',...
...
uc5=uicontrol('parent',compchoicepanel,'style','pushbutton','units','norm','pos',[.1 .7 .8 .2],...
    'string','GOOD','backgroundcolor','g','callback',@GoodComp_callback);
uc6=uicontrol('parent',compchoicepanel,'style','pushbutton','units','norm','pos',[.1 .4 .8 .2],...
    'string','UNSURE','backgroundcolor','y','callback',@UnsureComp_callback);
uc7=uicontrol('parent',compchoicepanel,'style','pushbutton','units','norm','pos',[.1 .1 .8 .2],...
...
compvars=squeeze(var(datain.cmp));
...
erpdata=mean(datain.cmp(:,:,goodtrials),3);
erpsep=(1000/ncomps)*nanmean(var(erpdata));
chansep=5*nanmean(nanmean(compvars(:,goodtrials)));
